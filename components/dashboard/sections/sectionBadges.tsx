...
export function getToneBadge(tone: Tone){
    switch(tone){
        ...
        case "friendly":
            return <Badge variant="outline" className="border-indigo-500/10 text-indigo-500  bg-indigo-500/5">Friendly</Badge>;
        case "empathetic":
...
export function getStatusBadge(status: SectionStatus){
    switch(status){
        case "active":
            return <Badge variant="success" className="bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/25">Active</Badge>;
        case "draft":
            return <Badge variant="secondary">Draft</Badge>
        case "disabled":
            return <Badge variant="outline" className="text-zinc-500 border-zinc-500/20 bg-zinc-500/5">Disabled</Badge>
    }
}