...
import { AlertCircle, Bot, ChevronDown, MessageCircle, MessageCircleIcon, Send } from 'lucide-react';
...
const EmbedPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchConfig = async () => {
            try {
                ...
                setMessages([{
                    role: "assistant",
                    content: data.metadata.welcome_messages || "Hi! How can I help you today?",
                    isWelcome: true,
                    section: null,
                }])
            } catch (error) {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if(res.ok){
                ...
                setMessages((prev) => [
                    ...prev,
                    {role: "assistant", content: data.answer ?? "No response.", section: null},
                ])
            }else{
               setMessages((prev) => [
                ...prev,
                {role: "assistant", content: "Something went wrong. Please try again.", section: null},
               ])
            }
        } catch (error) {
    ...
    if(error && isOpen) {
        return(
            <div className='flex flex-col items-center justify-center h-full bg-[#050509]'>
    ...
    if(!isOpen) {
        return(
            <button onClick={toggleOpen} className='w-14 h-14 rounded-full flex items-center justify-center shadow-lg hover:brightness-110 transition-all text-white' style={{backgroundColor: primaryColor}}>
        ...
        <div className='h-14 border-b border-white/5 flex items-center justify-between px-4 bg-[#0E0E12] shadow-sm shrink-0 z-20'>
            <div className='flex items-center gap-3'>
                <div className='relative'>
                    ...
                    <div className='absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border border-white/10'>
                    </div>
                </div>
                <div>
            ...
            className='p-2 text-zinc-400 hover:text-white hover:bg-white/10 rounded-lg transition-colors' aria-label='Minimize Chat'>
                <ChevronDown className='w-5 h-5'/>
            </button>
        </div>
        <div className='flex-1 min-h-0 overflow-y-auto bg-zinc-950/30 p-4 scrollbar-thin crollbar-thumb-white/10 scrollbar-track-transparent'>
                    ...
                    <div key={i} className={cn('flex w-full flex-col', msg.role === 'user' ? 'items-end' : 'items-start'
                        ...
                        <div className={cn('flex max-w-[85%] gap-3', msg.role === 'user' ? 'flex-row-reverse' : 'flex-row')}>
                            ...
                            <div className='space-y-2 min-w-0 max-w-full'>
                                <div className={cn('p-3 rounded-2xl text-xs leading-relaxed shadow-sm break-words overflow-hidden',
                                    msg.role === 'user' ? 'bg-zinc-800 text-zinc-100 rounded-tr-sm' : 'bg-white text-zinc-900 rounded-tl-sm'
                                )}>
                                    {msg.role === 'assistant' ? (
                                        <ReactMarkdown
                                 ...
                                 {msg.isWelcome && sections.length > 0 && (
                                    <div className='flex flex-wrap gap-2 pt-1 ml-1 animate-in fade-in slide-in-from-top-1 duration-300'>
                                        {sections.map((section) => (
                                            <button key={section.id} onClick={() => handleSectionClick(section.name)}
