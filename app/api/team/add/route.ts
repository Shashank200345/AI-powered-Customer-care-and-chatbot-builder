...
export async function POST(req: Request){
    try {
        ...
        const response = await scalekit.user.createUserAndMembership(LoggedInUser.organization_id, {
            email,
            userProfile: {
        ...
        // Return plain object; ScaleKit response.user is a protobuf Message and doesn't JSON-serialize safely
        const user = {
            id: response.user?.id,
        ...
        console.error("POST /api/team/add error:", error);
